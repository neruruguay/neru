...
$query = $SKTDB->get_row("SELECT * FROM PurchaseRequestss as PurchaseRequests join PurchaseRequestsprofile as profile WHERE PurchaseRequests.id = " . \GetSQLValueString($id, "int") . " AND PurchaseRequests.id = profile.idX");
...
<div class="CreateContentHtml">
    <form action="" method="post" id="Form_PurchaseRequestss">
        <input value="PurchaseRequestss_Edit" name="PurchaseRequestss_Edit" type="hidden"/>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        setTimeout('PurchaseRequestssHTML()', 1000);
    });

    function  PurchaseRequestssHTML() {
        var translations = [];
    ...
    $("#CmsDevDialogContent").dialog({
        ...
        buttons: {
            '[Save]': function () {
                var URLUPDATE = '/CRUD/ViewEditElementsAsList/Lists/PurchaseRequestss/_Update';
                jQuery.ajax({
                    'type': 'POST',
                    'url': '/SKTGoTo/' + admd2(URLUPDATE),
                    'cache': false,
                    'data': $("#Form_PurchaseRequestss").serialize(),
