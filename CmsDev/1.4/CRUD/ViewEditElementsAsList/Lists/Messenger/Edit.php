...
$query = $SKTDB->get_row("SELECT * FROM messenger as message join userprofile as profile WHERE message.id = " . \GetSQLValueString($id, "int") . " AND message.id = profile.idX");
...
<div class="CreateContentHtml">
    <form action="" method="post" id="Form_Messenger">
        <input value="Messenger_Edit" name="Messenger_Edit" type="hidden"/>
...
<script type="text/javascript">
    ...
    $("#CmsDevDialogContent").dialog({
        ...
        buttons: {
            '[Save]': function () {
                var URLUPDATE = '/CRUD/ViewEditElementsAsList/Lists/Messenger/_Update';
                jQuery.ajax({
                    'type': 'POST',
                    'url': '/SKTGoTo/' + admd2(URLUPDATE),
                    'cache': false,
                    'data': $("#Form_Messenger").serialize(),
