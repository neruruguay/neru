...
$HandleUploadDir = $_SERVER["DOCUMENT_ROOT"] . $subSite . '_FileSystems/CodeSnippets_Preview/';
$HandleUploadDir = str_replace('SKTGoTo/CRUD/ViewEditElementsAsList/Lists/ReusableComponent/Upload/', '', $HandleUploadDir);
...
class qqFileUploader {
    ...
    private function CheckFileNameImage($FileNameImage) {

        $nombre = utf8_decode(basename(trim($FileNameImage)));
    ...
    function handleUpload($uploadDirectory, $replaceOldFile = TRUE) {
        ...
        $nombre = utf8_decode(basename(trim($_GET['Params1'])));
        $ext = substr($nombre, strrpos($nombre, ".") + 1, 4);
        $nombre_correcto = str_replace($ext, "", $nombre);
        $nombre_correcto = str_replace("Cfakepath", "", $nombre_correcto);
        $nombre_correcto = str_replace(".", "", $nombre_correcto);
        ...
        if ($this->file->save($uploadDirectory . $filename . '.' . $ext)) {
            ...
            $uploadfile2 = $uploadDirectory . $nombre_Nuevo . '.' . $ext;
            if ($ext == 'jpg') {
                //$this->img_resizer($uploadfile1, 95, 250, 250, $uploadfile2, 'NocropImage');
            }
