...
$ezsql_mysql_str = array
    (
    1 => 'Require $SKTDBuser and $SKTDBpassword to connect to a database server',
    2 => 'Error establishing mySQL database connection. Correct user/password? Correct hostname? Database server running?',
    3 => 'Require $SKTDBname to select a database',
...
class ezSQL_mysql extends ezSQLcore {
    ...
    function ezSQL_mysql($SKTDBuser = '', $SKTDBpassword = '', $SKTDBname = '', $SKTDBhost = 'localhost') {
        $this->dbuser = $SKTDBuser;
        $this->dbpassword = $SKTDBpassword;
        $this->dbname = $SKTDBname;
        $this->dbhost = $SKTDBhost;
    }
    ...
    function quick_connect($SKTDBuser = '', $SKTDBpassword = '', $SKTDBname = '', $SKTDBhost = 'localhost') {
        $return_val = false;
        if (!$this->connect($SKTDBuser, $SKTDBpassword, $SKTDBhost, true))
            ;
        else if (!$this->select($SKTDBname))
            ;
        else
            $return_val = true;
        return $return_val;
    ...
    function connect($SKTDBuser = '', $SKTDBpassword = '', $SKTDBhost = 'localhost') {
        ...
        else if (!$this->dbh = @mysql_connect($SKTDBhost, $SKTDBuser, $SKTDBpassword, true, 131074)) {
    ...
    function select($SKTDBname = '') {
        ...
        else if (!@mysql_select_db($SKTDBname, $this->dbh)) {
    ...
    function query($query) {
        ...
        $this->func_call = "\$SKTDB->query(\"$query\")";
