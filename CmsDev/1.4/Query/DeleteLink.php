...
if (\CmsDev\Security\loginIntent::action('validateAdmin') === true) {
    ...
    $UPDATELink = $SKTDB->query(sprintf("UPDATE " . DB_PREFIX . "links 
					Set LinkTitle = %s, Target = %s, Link = %s, LinkType = %s, W = %s, H = %s, css_class = %s
					WHERE ID = %s", GetSQLValueString(utf8_decode($LinkTitle), "text"), GetSQLValueString($Target, "text"), GetSQLValueString($Link, "text"), GetSQLValueString($LinkType, "int"), GetSQLValueString($W, "int"), GetSQLValueString($H, "int"), GetSQLValueString($css_class, "text"), GetSQLValueString($ID, "int")
    ));
    if ($UPDATELink) {

        $UPDATE = $SKTDB->query(sprintf("UPDATE " . DB_PREFIX . "sections 
					Set Title = %s, URLName = %s, Position = %s
					DisplayOnMenu = %s,  
					WHERE Link_ID = %s", GetSQLValueString(utf8_decode($LinkTitle), "text"), GetSQLValueString($URLName, "text"), GetSQLValueString($Position, "int"), GetSQLValueString($DisplayOnMenu, "text"), GetSQLValueString($ID, "int")
        ));
