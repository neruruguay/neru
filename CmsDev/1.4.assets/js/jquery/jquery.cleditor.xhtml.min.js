 ...
 */
(function(k) {
    var n = k.cleditor.defaultOptions.updateTextArea;
    k.cleditor.defaultOptions.updateTextArea = function(a) {
        if (n)
            a = n(a);
        return k.cleditor.convertHTMLtoXHTML(a)
    };
    k.cleditor.convertHTMLtoXHTML = function(a) {
        function i(e) {
            var b = {};
            e = e.split(",");
        ...
        function v(e, b, f, h) {
            ...
            (h = z[b] || !!h) || c.push(b);
            var l = [];
            f.replace(A, function(D, m, o, p, q) {
                l.push({name: m, escaped: (o ?
                            o : p ? p : q ? q : B[m] ? m : "").replace(/(^|[^\\])"/g, '$1\\"')})
            });
            g += "<" + b;
            for (e = 0; e < l.length; e++)
                g += " " + l[e].name + '="' + l[e].escaped + '"';
            g += (h ? "/" : "") + ">"
        }
        function j(e, b) {
            if (b) {
        ...
        var s = /^<(\w+)((?:\s+\w+(?:\s*=\s*(?:(?:"[^"]*")|(?:'[^']*')|[^>\s]+))?)*)\s*(\/?)>/, t = /^<\/(\w+)[^>]*>/, A = /(\w+)(?:\s*=\s*(?:(?:"((?:\\.|[^"])*)")|(?:'((?:\\.|[^'])*)')|([^>\s]+)))?/g,
                ...
                B = i("checked,compact,declare,defer,disabled,ismap,multiple,nohref,noresize,noshade,nowrap,readonly,selected"), C = i("script,style"), c = [];
        c.last = function() {
        ...
        for (var d, u = a, g = ""; a; ) {
            if (!c.last() || !C[c.last()])
                if (a.indexOf("<!--") == 0) {
                    ...
                    d = a.indexOf("<");
                    g += d < 0 ? a : a.substring(0, d);
                    a = d < 0 ? "" : a.substring(d)
                }
            else {
                a = a.replace(RegExp("(.*)</" + c.last() + "[^>]*>"), function(e, b) {
