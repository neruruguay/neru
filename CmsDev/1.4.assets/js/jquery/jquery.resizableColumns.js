...
(function($, window) {
    ...
    ResizableColumns = (function() {
        ResizableColumns.prototype.defaults = {store: window.store, rigidSizing: false, resizeFromBody: true};
        ...
        ResizableColumns.prototype.syncHandleWidths = function() {
            ...
            return this.$handleContainer.find(".rc-handle").each(function(_, el) {
                return $(el).css({left: $(el).data("th").outerWidth() + ($(el).data("th").offset().left - _this.$handleContainer.offset().left), height: _this.options.resizeFromBody ? _this.$table.height() : _this.$table.find("thead").height()})
            })
        };
    ...
    return $.fn.extend({resizableColumns: function() {
            var args, option;
